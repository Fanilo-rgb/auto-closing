import { create } from "zustand"
import type { product } from "../types/type.ts"

type StoreType = {
  products: product[];
  getProducts: () => void;
  setProducts: (data: product[]) => void;
  setProduct: (data: product) => void;
}

export const useProductStore = create<StoreType>((set) => ({
  products: [],
  getProducts: async () => {
    const productList: {
      name: string;
      bv: number;
      price: number | null;
      order: number;
    }[] =
      [
        {
          "name": "Cordyceps Plus Capsule",
          "bv": 20,
          "price": null,
          "order": 1
        },
        {
          "name": "Spirulina Plus Capsule",
          "bv": 21,
          "price": null,
          "order": 2
        },
        {
          "name": "Propolis Plus Capsule",
          "bv": 27,
          "price": null,
          "order": 3
        },
        {
          "name": "Ganoderma Plus Capsule",
          "bv": 23,
          "price": null,
          "order": 4
        },
        {
          "name": "Cardio Power Capsule",
          "bv": 19,
          "price": null,
          "order": 5
        },
        {
          "name": "Soy Power Capsule",
          "bv": 18,
          "price": null,
          "order": 6
        },
        {
          "name": "A-Power Capsule",
          "bv": 40,
          "price": null,
          "order": 7
        },
        {
          "name": "Vig Power Capsule",
          "bv": 26,
          "price": null,
          "order": 8
        },
        {
          "name": "Ginseng RHs Capsule",
          "bv": 38,
          "price": null,
          "order": 9
        },
        {
          "name": "Pro-slim Tea",
          "bv": 12,
          "price": null,
          "order": 10
        },
        {
          "name": "Kuding Plus Tea",
          "bv": 12,
          "price": null,
          "order": 11
        },
        {
          "name": "Pine Pollen Tea",
          "bv": 14,
          "price": null,
          "order": 12
        },
        {
          "name": "Intestine Cleansing Tea",
          "bv": 13,
          "price": null,
          "order": 13
        },
        {
          "name": "Balsam Pear Tea",
          "bv": 13,
          "price": null,
          "order": 14
        },
        {
          "name": "Lipid Care Tea",
          "bv": 14,
          "price": null,
          "order": 15
        },
        {
          "name": "Breast Care Tea",
          "bv": 15,
          "price": null,
          "order": 16
        },
        {
          "name": "Ganoderma Coffee",
          "bv": 18,
          "price": null,
          "order": 17
        },
        {
          "name": "kidney Tonifying capusle man",
          "bv": 30,
          "price": null,
          "order": 18
        },
        {
          "name": "kidney Tonifying capusle women",
          "bv": 30,
          "price": null,
          "order": 19
        },
        {
          "name": "Protein Powder",
          "bv": 35,
          "price": null,
          "order": 20
        },
        {
          "name": "Multi-vitamin Tablet (for children)",
          "bv": 8.5,
          "price": null,
          "order": 21
        },
        {
          "name": "Multi-vitamin Tablet (for adults)",
          "bv": 23,
          "price": null,
          "order": 22
        },
        {
          "name": "Chewable Calcium Tablet (for children)",
          "bv": 8.5,
          "price": null,
          "order": 23
        },
        {
          "name": "Calcium Softgel (for adults)",
          "bv": 22,
          "price": null,
          "order": 24
        },
        {
          "name": "Zinc Tablet (for children)",
          "bv": 9,
          "price": null,
          "order": 25
        },
        {
          "name": "Zinc Tablet (for adults)",
          "bv": 16,
          "price": null,
          "order": 26
        },
        {
          "name": "Soybean Lecithin Softgel",
          "bv": 20,
          "price": null,
          "order": 27
        },
        {
          "name": "Deep Sea Fish Oil Softgel (Omega-3)",
          "bv": 25,
          "price": null,
          "order": 28
        },
        {
          "name": "Garlic Oil Softgel",
          "bv": 22.5,
          "price": null,
          "order": 29
        },
        {
          "name": "Eye Care Softgel",
          "bv": 28,
          "price": null,
          "order": 30
        },
        {
          "name": "Chitosan Capsule",
          "bv": 24,
          "price": null,
          "order": 31
        },
        {
          "name": "Aloe Vera Plus Capsule",
          "bv": 21,
          "price": null,
          "order": 32
        },
        {
          "name": "Compound Marrow Powder",
          "bv": 26,
          "price": null,
          "order": 33
        },
        {
          "name": "Ginkgo Biloba Capsule",
          "bv": 22.5,
          "price": null,
          "order": 34
        },
        {
          "name": "FLASK",
          "bv": 70,
          "price": null,
          "order": 35
        },
        {
          "name": "Clear Lung Tea",
          "bv": 14,
          "price": null,
          "order": 36
        },
        {
          "name": "Royal Jelly Softgel",
          "bv": 18.5,
          "price": null,
          "order": 37
        },
        {
          "name": "hepatsure capsule",
          "bv": 26,
          "price": null,
          "order": 38
        },
        {
          "name": "Ishine Capsule",
          "bv": 22,
          "price": null,
          "order": 39
        },
        {
          "name": "Parashield Capsule",
          "bv": 15,
          "price": null,
          "order": 40
        },
        {
          "name": "Slimming Capsule",
          "bv": 28,
          "price": null,
          "order": 41
        },
        {
          "name": "Super Nutrition  Powder",
          "bv": 46,
          "price": null,
          "order": 42
        },
        {
          "name": "ProstaSure capsule",
          "bv": 33,
          "price": null,
          "order": 43
        },
        {
          "name": "CoQ-10 Capsule",
          "bv": 27,
          "price": null,
          "order": 44
        },
        {
          "name": "Bone Care Plaster",
          "bv": 20,
          "price": null,
          "order": 45
        },
        {
          "name": "Diasure Capsule",
          "bv": 18,
          "price": null,
          "order": 46
        },
        {
          "name": "Joint Health Capsule",
          "bv": 28,
          "price": null,
          "order": 47
        },
        {
          "name": "Vitamin C Tablet",
          "bv": 12,
          "price": null,
          "order": 48
        },
        {
          "name": "Digestant Tablet(Gastric Health)",
          "bv": 16,
          "price": null,
          "order": 49
        },
        {
          "name": "Magic Detoxin Pad",
          "bv": 20,
          "price": null,
          "order": 50
        },
        {
          "name": "Blueberry Juice Granules",
          "bv": 22,
          "price": null,
          "order": 51
        },
        {
          "name": "Anti-aging  Capusle",
          "bv": 32,
          "price": null,
          "order": 52
        },
        {
          "name": "Blueberry Enzymes Tablet",
          "bv": 23,
          "price": null,
          "order": 53
        },
        {
          "name": "Hangfang Beauty Milk Soap",
          "bv": 1,
          "price": 6600,
          "order": 54
        },
        {
          "name": "Quick Absorption Blue Sanitary Napkin",
          "bv": 2,
          "price": 10000,
          "order": 55
        },
        {
          "name": "Toothpaste",
          "bv": 7.5,
          "price": null,
          "order": 56
        },
        {
          "name": "BLUEBERRY SERIES",
          "bv": 200,
          "price": null,
          "order": 57
        },
        {
          "name": "Uterus Cleansing Pill",
          "bv": 32,
          "price": null,
          "order": 58
        },
        {
          "name": "Mala Power",
          "bv": 15,
          "price": null,
          "order": 59
        },
        {
          "name": "Chang Jing Jing",
          "bv": 10,
          "price": null,
          "order": 60
        },
        {
          "name": "Jiu Qing Qing",
          "bv": 10,
          "price": null,
          "order": 61
        },
        {
          "name": "Smilife Skin Care Package",
          "bv": 40,
          "price": null,
          "order": 62
        },
        {
          "name": "VITAMIN E",
          "bv": 21,
          "price": null,
          "order": 63
        },
        {
          "name": "β-CAROTENE & LYCOPENE CAPSULE",
          "bv": 22,
          "price": null,
          "order": 64
        },
        {
          "name": "LIVERGEN CAPSULE",
          "bv": 28,
          "price": null,
          "order": 65
        },
        {
          "name": "SE TABLET(ADULTS)",
          "bv": 18.5,
          "price": null,
          "order": 66
        },
        {
          "name": "SE TABLET(CHILDREN)",
          "bv": 8.5,
          "price": null,
          "order": 67
        },
        {
          "name": "Blueberry milk",
          "bv": 17,
          "price": null,
          "order": 68
        },
        {
          "name": "JINPURE CAPSULE",
          "bv": 22,
          "price": null,
          "order": 69
        },
        {
          "name": "JINPURE TEA",
          "bv": 13,
          "price": null,
          "order": 70
        },
        {
          "name": "OVARY CAPSULE",
          "bv": 24,
          "price": null,
          "order": 71
        },
        {
          "name": "ENERGY DRINK",
          "bv": 10,
          "price": null,
          "order": 72
        },
        {
          "name": "ENERGY TEA",
          "bv": 15,
          "price": null,
          "order": 73
        },
        {
          "name": "ENGRAIS NUTRIPLANT",
          "bv": 30,
          "price": null,
          "order": 74
        },
        {
          "name": "SLIMMING MACHINE",
          "bv": 70,
          "price": null,
          "order": 75
        },
        {
          "name": "KNEE PROTECTOR",
          "bv": 45,
          "price": null,
          "order": 76
        },
        {
          "name": "INFRARED MACHIE",
          "bv": 70,
          "price": null,
          "order": 77
        },
        {
          "name": "COUMPOUND CALCIUM AD",
          "bv": 22,
          "price": null,
          "order": 78
        },
        {
          "name": "COUMPOUND CALCIUM CHILD",
          "bv": 20,
          "price": null,
          "order": 79
        },
        {
          "name": "EVA SPRAY WOMEN ",
          "bv": 17,
          "price": null,
          "order": 80
        },
        {
          "name": "GOLD SPRAY MEN ",
          "bv": 18,
          "price": null,
          "order": 81
        },
        {
          "name": "MEAL CELLULOSE TABLETS",
          "bv": 26,
          "price": null,
          "order": 82
        },
        {
          "name": "ANTI ADDICTION CAPS",
          "bv": 35,
          "price": null,
          "order": 83
        },
        {
          "name": "BLOOD CLEANSE CAPS",
          "bv": 18,
          "price": null,
          "order": 84
        },
        {
          "name": "TOURMALINE SOAP ",
          "bv": 1,
          "price": 7000,
          "order": 85
        },
        {
          "name": "COMPOUND CALCIUM VERT",
          "bv": 22,
          "price": null,
          "order": 86
        },
        {
          "name": "NMN",
          "bv": 60,
          "price": null,
          "order": 87
        },
        {
          "name": "VD",
          "bv": 23,
          "price": null,
          "order": 88
        },
        {
          "name": "BITTER BUCKWHEAT TEA",
          "bv": 12,
          "price": null,
          "order": 89
        },
        {
          "name": "BRAIN CARE CAPSULE",
          "bv": 28,
          "price": null,
          "order": 90
        },
        {
          "name": "PROBIOTIC",
          "bv": 12,
          "price": null,
          "order": 91
        },
        {
          "name": "PRESSURE SPRAY",
          "bv": 10,
          "price": null,
          "order": 92
        },
        {
          "name": "BLUEBERRY CONCENTRATE",
          "bv": 85,
          "price": null,
          "order": 93
        },
        {
          "name": "SEE",
          "bv": 10,
          "price": null,
          "order": 94
        },
        {
          "name": "ENERGY STONE PENDANT",
          "bv": 55,
          "price": null,
          "order": 95
        },
        {
          "name": "BLUEBERRY SLIMMING BODY",
          "bv": 42,
          "price": null,
          "order": 96
        },
        {
          "name": "BLUEBERRY WINE",
          "bv": 40,
          "price": null,
          "order": 97
        }
      ]

    const correctedProducts = productList.map((product) => ({
      ...product,
      quantity: 0,
      price: product.price ? product.price : product.bv * 3600
    }))

    set({ products: correctedProducts })
  },
  setProducts: (data) => set({ products: data }),
  setProduct: (data) => {
    set((state) => ({
      products: state.products.map((p) =>
        p.name === data.name ? { ...p, ...data } : p
      ),
    }))
  }
}))
